...
Triage Range simula el trabajo del día a día de un analista SOC Tier 1: recibís una alerta con contexto mínimo, tenés que filtrar entre un volumen de logs (una mezcla de actividad real y ruido intencional), cruzar lo que encontrás con el contexto de la empresa, y llegar a una conclusión respaldada por evidencia — no por instinto.

No es una lista de preguntas de trivia sobre ciberseguridad. Es un entorno de práctica donde la única forma de responder bien es investigar de verdad.

Cómo funciona
Elegís un caso por dificultad (Fácil / Medio / Difícil), cada uno con su propio escenario, volumen de logs y objetivo.
Filtrás los logs con sintaxis de búsqueda tipo DQL (campo:"valor", AND/OR/AND NOT, rangos >=/<=), igual que en Wazuh Discover — con un panel de campos dinámico para armar la consulta sin tener que memorizar la sintaxis desde cero.
Verificás contexto en el panel de Contexto SOC: buscás cualquier IP, hostname, usuario, correo o hash y el sistema te dice si es un activo interno, un usuario del directorio, una herramienta/SaaS interno autorizado, o infraestructura de amenaza conocida — igual que haría un analista real antes de escalar o descartar una alerta. También funciona con click to pivot: clic directo sobre cualquier valor dentro de un log para consultarlo al instante, sin copiar y pegar.
Respondés con evidencia: cada caso tiene 10 preguntas verificables contra los logs y el contexto investigado, con pistas progresivas si te trabás.
Cerrás el caso: en dificultad Fácil, reconstruyendo la cadena de ataque completa (Kill Chain, fase por fase). En Medio y Difícil, con un veredicto final — ¿Falso Positivo o Verdadero Positivo? — porque en un SOC real, descartar bien lo que no es un ataque es tan importante como detectar lo que sí lo es.
...
José Corrales — jcorrales02.github.io/Jcorrales-Web · LinkedIn · GitHub
